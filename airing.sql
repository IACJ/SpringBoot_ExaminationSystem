...
(101, '对并发操作若不加以控制，可能会带来                     问题。', '不安全', '死锁', '死机', '不一致  ', 'D', 0, 11),
(102, '并发操作会带来哪些数据不一致性                      。', '丢失修改、不可重复读、脏读、死锁', '不可重复读、脏读、死锁 ', '丢失修改、脏读、死锁', '丢失修改、不可重复读、脏读 ', 'D', 0, 11);
...
--
ALTER TABLE `class`
  ADD CONSTRAINT `class_ibfk_1` FOREIGN KEY (`tid`) REFERENCES `teacher` (`id`);
...
--
ALTER TABLE `status`
  ADD CONSTRAINT `status_ibfk_1` FOREIGN KEY (`uid`) REFERENCES `student` (`id`);
